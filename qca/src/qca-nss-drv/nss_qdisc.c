					...
					struct nss_shaper_response *response)
{
	...
	nssqdisc_info("%s: Root qdisc %p (type %d) shaper unsassign "
		"response: %d\n", __func__, nq->qdisc, nq->type, response->type);
				...
				struct nss_shaper_response *response)
{
	...
	shaper_node_create.app_data = nq;
				...
				struct nss_shaper_response *response)
{
	...
	nssqdisc_info("%s: Qdisc %p (type %d qos_tag %x): child free response type: %d\n",
			__func__, nq->qdisc, nq->type, nq->qos_tag, response->type);
...
static int nssqdisc_set_default(struct nssqdisc_qdisc *nq)
{
	...
	shaper_node_default.app_data = nq;
	...
	struct nss_shaper_configure *shaper_node_attach, int32_t attach_type)
{
	...
	shaper_node_attach->app_data = nq;
	...
	struct nss_shaper_configure *shaper_node_detach, int32_t detach_type)
{
	...
	shaper_node_detach->app_data = nq;
	...
	struct nss_shaper_configure *shaper_node_configure, int32_t config_type)
{
	...
	shaper_node_configure->app_data = nq;
...
static void nssqdisc_destroy(struct nssqdisc_qdisc *nq)
{
	...
	if (nq->is_root) {
		...
		if (nq->is_bridge) {
			...
			nssqdisc_refresh_bshaper_assignment(nq->qdisc, NSSQDISC_UNASSIGN_BSHAPER);
		}
...
static int nssqdisc_init(struct Qdisc *sch, struct nssqdisc_qdisc *nq, nss_shaper_node_type_t type, uint32_t classid)
{
	...
	if (state > 0) {
		 ...
		 */
		if (nssqdisc_refresh_bshaper_assignment(nq->qdisc, NSSQDISC_ASSIGN_BSHAPER) < 0) {
			nssqdisc_destroy(nq);
...
static int nssfifo_change(struct Qdisc *sch, struct nlattr *opt)
{
	...
	if (nssqdisc_configure(&q->nq, &shaper_node_change_param, NSS_SHAPER_CONFIG_TYPE_FIFO_CHANGE_PARAM) < 0) {
...
static int nsscodel_change(struct Qdisc *sch, struct nlattr *opt)
{
	...
	shaper_node_change_param.mt.shaper_node_config.snc.codel_param.cap.mtu = psched_mtu(dev);
	nssqdisc_info("%s: MTU size of interface %s is %u bytes\n", __func__, dev->name,
			shaper_node_change_param.mt.shaper_node_config.snc.codel_param.cap.mtu);

	if (nssqdisc_configure(&q->nq, &shaper_node_change_param,
...
static void nsstbl_destroy(struct Qdisc *sch)
{
	...
	struct nss_shaper_configure shaper_node_detach;
	...
	if (q->qdisc != &noop_qdisc) {
		...
		if (nssqdisc_node_detach(&q->nq, &shaper_node_detach,
			...
			struct Qdisc **old)
{
	...
	struct nss_shaper_configure shaper_node_attach;
	struct nss_shaper_configure shaper_node_detach;
	...
	*old = q->qdisc;
	...
	if (new != &noop_qdisc) {
		...
		if (nssqdisc_node_attach(&q->nq, &shaper_node_attach,
...
static void nssprio_destroy(struct Qdisc *sch)
{
	...
	for (i = 0; i < q->bands; i++) {
		...
		if (q->queues[i] != &noop_qdisc) {
			shaper_node_detach.mt.shaper_node_config.qos_tag = q->nq.qos_tag;
			shaper_node_detach.mt.shaper_node_config.snc.prio_detach.priority = i;
			if (nssqdisc_node_detach(&q->nq, &shaper_node_detach,
		  ...
		  struct nlattr **tca, unsigned long *arg)
{
	...
	int err;
	struct nss_shaper_configure shaper_node_change_param, shaper_node_attach;
	struct net_device *dev = qdisc_dev(sch);
	...
	if (!cl) {
		...
		*arg = (unsigned long)cl;
		 ...
		 */
		shaper_node_attach.mt.shaper_node_config.snc.bf_attach.child_qos_tag = cl->nq.qos_tag;
	 ...
	 * bf class.
	 */
	shaper_node_change_param.mt.shaper_node_config.qos_tag = cl->nq.qos_tag;
	shaper_node_change_param.mt.shaper_node_config.snc.bf_group_param.quantum = cl->quantum;
	shaper_node_change_param.mt.shaper_node_config.snc.bf_group_param.lap.rate = cl->rate;
	shaper_node_change_param.mt.shaper_node_config.snc.bf_group_param.lap.burst = cl->burst;
	shaper_node_change_param.mt.shaper_node_config.snc.bf_group_param.lap.max_size = cl->mtu;
	shaper_node_change_param.mt.shaper_node_config.snc.bf_group_param.lap.short_circuit = false;
	 ...
	 */
	if (nssqdisc_configure(&cl->nq, &shaper_node_change_param,
...
static void nssbf_destroy_class(struct Qdisc *sch, struct nssbf_class_data *cl)
{
	...
	if (cl->qdisc != &noop_qdisc) {
		shaper_node_detach.mt.shaper_node_config.qos_tag = cl->nq.qos_tag;
		if (nssqdisc_node_detach(&cl->nq, &shaper_node_detach,
...
static int nssbf_delete_class(struct Qdisc *sch, unsigned long arg)
{
	...
	shaper_node_detach.mt.shaper_node_config.qos_tag = q->nq.qos_tag;
	shaper_node_detach.mt.shaper_node_config.snc.bf_detach.child_qos_tag = cl->nq.qos_tag;
	if (nssqdisc_node_detach(&q->nq, &shaper_node_detach,
								 ...
								 struct Qdisc **old)
{
	...
	*old = cl->qdisc;
	...
	if (new != &noop_qdisc) {
		nssqdisc_info("%s: Attaching new: %p\n", __func__, new);
		shaper_node_attach.mt.shaper_node_config.qos_tag = cl->nq.qos_tag;
		shaper_node_attach.mt.shaper_node_config.snc.bf_group_attach.child_qos_tag = nq_new->qos_tag;
		if (nssqdisc_node_attach(&cl->nq, &shaper_node_attach,
